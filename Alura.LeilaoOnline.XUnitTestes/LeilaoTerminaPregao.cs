...
namespace Alura.LeilaoOnline.XUnitTestes
{
    public class LeilaoTerminaPregao
    {
        ...
        [InlineData(1200, new double[] { 900, 1000, 1200, 1100 })]
        [InlineData(700, new double[] { 700 })]
        public void RetornaLanceComValorSuperiorMaisProximoDadoPeloMenosUmLance(double valorObtido, double[] lances)
        {
            //arrange
            var avaliacao = new OfertaSuperiorMaisProxima(valorObtido);
            var leilao = new Leilao("Van Gogh", avaliacao);
            var fulano = new Interessada("Fulano", leilao);
            var maria = new Interessada("Maria", leilao);
            leilao.IniciaPregao();
        ...
        public void LancaInvalidOperationExceptionDadoTerminaPregaoSemInicio()
        {
            //arrange
            var avaliacao = new MaiorValor();
            var leilao = new Leilao("Auto da compadecida", avaliacao);
            //assert
